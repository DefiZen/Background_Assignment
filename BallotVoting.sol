 ...
 * @dev Limits voting period for each ballot to 5 mins 
 */
contract BallotVoting {
    ...
    enum State { created, startTime, Voting, voteEnded } // State of voting period
    ...
    modifier StartTimeState() {
        require(state == State.startTime, "it must be in Started");
        _;
    }
   ...
   function endVote() 
    ...
    function vote(address voter) public {
        ...
        require(
            !voters[voter].voteEnded,
            "Voting Forbiden , Voting window opens for 5 minutes only"
        );
        ...
        VotingState
    {
        ...
        require(!sender.voteEnded," Forbiden, Votting window open for 5 mins only");
        sender.voted = true;
